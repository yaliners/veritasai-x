...
import { FileText, Shield, AlertOctagon, TrendingUp, Calendar, Clock, Award, Printer, CheckCircle } from "lucide-react";
...
export const Route = createFileRoute("/report")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Weekly threat summary, security score analysis, and personalized insights." },
    ],
  }),
...
function SecurityReport() {
  ...
  const reportData = useMemo(() => {
    ...
    if (securityScore >= 90) {
      recommendations.push("Your browsing hygiene is outstanding! Keep following the same patterns to stay secure.");
    } else if (securityScore >= 70) {
      recommendations.push("Good safety score, but minor alerts occurred. Be sure to check domains with multiple hyphens or non-standard TLD extensions.");
    } else {
      recommendations.push("Critical security alerts detected. Avoid visiting sites without HTTPS encryption or entering data on freshly-registered domains.");
    }

    if (weeklyScans.some((t) => t.reasons.some((r) => r.toLowerCase().includes("http:") || r.toLowerCase().includes("ssl")))) {
      recommendations.push("Warning: Secure Socket Layer (SSL) anomalies detected. Avoid transmitting bank accounts or credit card details on unencrypted HTTP connections.");
    }

    if (weeklyScans.some((t) => t.module === "New Domain — High Risk" || t.module === "New Domain")) {
      recommendations.push("Ensure to verify domain addresses for recent registration dates. Scammers frequently buy domains and discard them in under 30 days.");
    }

    if (threatsBlocked > 0) {
      recommendations.push("Our AI engines blocked active phishing threats. We recommend running a full scan of your operating system with a verified antivirus provider.");
    }
  ...
  return (
    <>
      <div className="print:hidden">
        <Topbar title="Weekly Security Report" subtitle="Auto-generated intelligence briefing and safety scorecard" />
      </div>
        ...
        {!isExtensionInstalled ? (
          ...
          <div className="glass rounded-2xl p-12 text-center flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              Not enough data to compile this week's report. Start browsing websites with the extension active to build threat statistics.
            </p>
          </div>
        ) : (
          <div className="space-y-6 max-w-5xl mx-auto">
            ...
            <div className="hidden print:block mb-8 border-b pb-6 border-slate-300">
              ...
              <p className="text-sm text-slate-500 uppercase tracking-widest mt-1">Weekly Security & Threat Telemetry Briefing</p>
              <p className="text-xs text-slate-400 mt-0.5">Generated on: {new Date().toLocaleDateString()} at {new Date().toLocaleTimeString()}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="glass rounded-2xl p-6 flex flex-col justify-between items-center text-center border-l-4 border-l-cyber-cyan min-h-[220px]">
                ...
                <div className="my-4">
                  <span className="text-5xl md:text-6xl font-extrabold font-mono text-cyber-cyan">{reportData.securityScore}</span>
                  <span className="text-sm text-muted-foreground">/100</span>
                </div>
                <span className={`text-xs px-3 py-1 rounded-full font-bold uppercase ${
                  reportData.securityScore >= 90 ? "bg-cyber-success/12 text-cyber-success border border-cyber-success/20" :
                  reportData.securityScore >= 70 ? "bg-cyber-warning/12 text-cyber-warning border border-cyber-warning/20" :
                  "bg-cyber-danger/12 text-cyber-danger border border-cyber-danger/20"
                }`}>
                  {reportData.securityScore >= 90 ? "Excellent" :
                   reportData.securityScore >= 70 ? "Fair" : "At Risk"}
                </span>
              </div>
              ...
              <div className="glass rounded-2xl p-6 md:col-span-2 grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-card/30 border border-border/40 flex flex-col justify-between">
                  ...
                  <p className="text-3xl font-extrabold font-mono mt-2 text-foreground">{reportData.totalScanned}</p>
                ...
                <div className="p-4 rounded-xl bg-card/30 border border-border/40 flex flex-col justify-between">
                  ...
                  <p className="text-3xl font-extrabold font-mono mt-2 text-cyber-danger">{reportData.threatsBlocked}</p>
                ...
                <div className="p-4 rounded-xl bg-card/30 border border-border/40 flex flex-col justify-between">
                  ...
                  <p className="text-xl font-extrabold mt-2 text-foreground truncate">{reportData.peakDay}</p>
                ...
                <div className="p-4 rounded-xl bg-card/30 border border-border/40 flex flex-col justify-between">
                  ...
                  <p className="text-base font-extrabold mt-2 text-foreground truncate">{reportData.peakTimeStr}</p>
                  <p className="text-[10px] text-muted-foreground mt-1">Highest threat score block</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="glass rounded-2xl p-6 space-y-4">
                <h3 className="text-base font-semibold border-b border-border/40 pb-2">Threat Vector Summary</h3>
                <div className="space-y-3.5">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Top Threat Category:</span>
                    <span className="font-semibold text-cyber-warning">{reportData.topThreatCategory}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Suspicious Domains Encountered:</span>
                    <span className="font-semibold text-foreground">{reportData.suspiciousEncountered}</span>
                  </div>
                  <div className="flex justify-between items-start text-sm">
                    ...
                    <div className="text-right">
                      {reportData.mostDangerousSite ? (
                        <>
                          <p className="font-mono text-xs font-semibold text-cyber-danger truncate max-w-[200px]">{reportData.mostDangerousSite.domain}</p>
                          <p className="text-[10px] text-muted-foreground">Score: {reportData.mostDangerousSite.score} ({reportData.mostDangerousSite.module})</p>
                        </>
                      ) : (
              ...
              <div className="glass rounded-2xl p-6 space-y-4">
                ...
                <ul className="space-y-3">
                  {reportData.recommendations.map((rec, idx) => (
                    <li key={idx} className="text-xs flex gap-2.5 text-foreground/90 leading-relaxed">

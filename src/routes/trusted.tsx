...
import { useTrustedSites, useExtensionInstalled } from "@/lib/veritas/store";
...
function autoCategorize(inputDomain: string): string {
  ...
  if (
    ...
    ["government", "gov", "state", "federal", "national", "admin"].some((x) => clean.includes(x))
  )
    return "Government";
  if (
    clean.endsWith(".org") ||
    ["charity", "nonprofit", "ngo", "foundation", "redcross", "unicef"].some((x) =>
      clean.includes(x),
    )
  )
...
function TrustedManager() {
  ...
  function add() {
    ...
    if (!clean) return;
    setSites([
      { id: `t_${Date.now()}`, domain: clean, category, trustLevel: level, addedAt: Date.now() },
      ...sites,
    ]);
    setDomain("");
    setCategory("General");
  ...
  return (
    <>
      <Topbar
        title="Trusted Sites Manager"
        subtitle="Whitelisted domains bypass all detection engines"
      />
          ...
          <>
            <section className="glass rounded-2xl p-6 shadow-[var(--shadow-card)]">
              ...
              <p className="mb-4 text-xs text-muted-foreground">
                Domains added here become Risk: TRUSTED · Score: 0 · Trust: 100.
              </p>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-[2fr_1fr_1fr_auto]">
                ...
                <select
                  value={level}
                  onChange={(e) => setLevel(e.target.value as TrustedSite["trustLevel"])}
                ...
                <button
                  onClick={add}
                  className="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-cyber-cyan to-primary px-4 py-2 text-sm font-semibold text-background"
                >
                      ...
                      <tr key={s.id} className="border-b border-border/30 last:border-0">
                        <td className="py-3">
                          <div className="flex items-center gap-2 mono text-xs truncate max-w-[120px] sm:max-w-[240px] md:max-w-[320px]">
                            <ShieldCheck className="h-3.5 w-3.5 text-cyber-success shrink-0" />{" "}
                            {s.domain}
                          </div>
                        </td>
                        ...
                        <td>
                          <span
                            className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${levelCls[s.trustLevel]}`}
                          >
                            {s.trustLevel}
                          </span>
                        </td>
                        ...
                        <td className="text-right">
                          <button
                            onClick={() => remove(s.id)}
                            className="inline-flex items-center gap-1 rounded-md border border-cyber-danger/40 bg-cyber-danger/10 px-2 py-1 text-[10px] font-semibold text-cyber-danger hover:bg-cyber-danger/20"
                          >
